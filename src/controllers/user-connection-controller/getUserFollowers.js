...
const getUserFollowers = async (req, res) => {
  try {
    ...
    const userFollowersCount = UserConnection.find(query[0].$match).count();

    const userFollowers = UserConnection.aggregate([
      ...query,
      {
        $project: { status: 0 },
      },
    ...
    Promise.all([userFollowersCount, userFollowers])
      .then((resData) => {
